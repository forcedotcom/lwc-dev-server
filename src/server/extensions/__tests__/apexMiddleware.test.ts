...
function getMiddleware(
    ...
    const extension = apexMiddleware(connection);
...
describe('apexMiddleware', () => {
    ...
    it('ignores non apex call ', async () => {
        const middleware = getMiddleware(mockConnection, app, options);
    ...
    it('apex call has sid cookie', async () => {
        mockConnection.timeout = 0;

        const middleware = getMiddleware(mockConnection, app, options);
        const req: any = {
            url: '/webruntime/api/apex/execute',
            body: {
                classname: 'classname',
                method: 'method',
                namespace: 'namespace',
