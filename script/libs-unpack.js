...
if (process.env.SKIP_INSTALL_LIBS) {
    doRun = false;
} else if (args.length === 0 || installcmds.includes(args[0])) {
    doRun = true;
} else if (
    args.length > 2 &&
    args[0] === 'add' &&
...
(async () => {
    try {
        ...
        console.error('libs-unpack took ' + (end - start) + ' ms');
    } catch (e) {
        console.error(`unable to unpack local libs - ${e}`);
        process.exit(1);
    }
})();
...
async function writePackageInfo(packages) {
    const dest = path.join(unpackPath, 'packages.json');
    fs.writeFileSync(dest, JSON.stringify(packages, null, 2), 'utf8');
