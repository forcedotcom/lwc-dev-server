...
function renderTemplate(isCompat, useCustomElementRegistry, useNativeShadow) {
    ...
    return template
        .replace(
            '{{useCustomElementRegistry}}',
            useCustomElementRegistry ? 'enabled' : 'disabled'
        )
        .replace(
            '{{useNativeShadow}}',
            useNativeShadow ? 'enabled' : 'disabled'
        )
...
app.get('/', (req, res) => {
    ...
    res.send(
        renderTemplate(isCompatMode, useCustomElementRegistry, useNativeShadow)
    );
});
