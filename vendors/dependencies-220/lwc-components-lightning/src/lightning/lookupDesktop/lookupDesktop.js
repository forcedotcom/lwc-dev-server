...
export default class LightningLookupDesktop extends LightningElement {
     ...
     *
     */
    @api messageWhenBadInput = utils.computeMessageWhenBadInputDefault();
    ...
    /**
     * An array of selected values for the selected lookup.
     * @type {Array}
     */
    _value;
    ...
    wiredRecordUi({ error, data }) {
        ...
        try {
            ...
            if (invalidValues.length > 0 && this._value) {
                ...
                const values = this._value.filter(
    ...
    createNewCallback(values = []) {
        ...
        const newValues = Object.assign([], this._value).concat(values);
    ...
    handleRecordOptionSelect(recordId) {
        ...
        if (
            Array.isArray(this._value) &&
            this._value.length === this._maxValues
        ) {
            return;
        }
    ...
    updateMaxValues(count) {
        ...
        if (this._value.length > this._maxValues) {
            const values = this._value.slice(0, this._maxValues);
            // Trim existing pills as per new max count.
            const pillValues = this._pills.map(pill => {
                return pill.value;
            });

            if (utils.arraysIdentical(pillValues, this._value)) {
    ...
    updatePills(pills = []) {
        ...
        let pillsResolved = utils.arraysIdentical(pillValues, this._value);
        ...
        if (!pillsResolved) {
            try {
                ...
                if (utils.arraysIdentical(values, this._value)) {
    ...
    updateState() {
        ...
        try {
            ...
            // If field is marked as required in the field info then override the public property.
            if (this._fieldInfo.isRequired) {
                this._required = this._fieldInfo.isRequired;
            }

            this.isRequired = this._required;
    ...
    updateValue(value = [], pills = [], triggerEvent = true) {
        ...
        if (utils.arraysIdentical(value, this._value)) {
            return;
        }
