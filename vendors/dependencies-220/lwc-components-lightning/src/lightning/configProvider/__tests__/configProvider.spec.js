...
describe('lightning-configProvider', () => {
    ...
    describe('providing a configProvider', () => {
        const configProviderMock = {
            ...
            getInitializer(info) {
                if (info === 'coreInfoConfig') {
                    return {
                        untrustedContentDomain: '.a.forceusercontent.com',
                        localhostPort: '8080',
                        securePort: '',
                    };
                } else if (info === 'oneConfig') {
                    return {
        ...
        it('should override with provided values', () => {
            ...
            expect(getCoreInfo().localhostPort).toBe(
                configProviderMock.getInitializer('coreInfoConfig')
                    .localhostPort
            );
            expect(getOneConfig().densitySetting).toBe(
                configProviderMock.getInitializer('oneConfig').densitySetting
            );
        ...
        it('should allow to override a single entry', () => {
            const configProviderModule = require('../configProvider');
            const configProviderService = configProviderModule.default;
            const {
                getFormFactor,
            ...
            expect(getFormFactor()).toBe('test');
            expect(getToken(token)).toBe(token);
            expect(getPathPrefix()).toBe('');
        });
        it('should allows to inject iconSvgTemplates', () => {
            const configProviderModule = require('../configProvider');
            const configProviderService = configProviderModule.default;
            const { getIconSvgTemplates } = configProviderModule;
            const iconSvgTemplates = { utilityDown: () => {} };
            configProviderService({
                iconSvgTemplates,
            });
            expect(getIconSvgTemplates()).toEqual(iconSvgTemplates);
        });
        it('should use default value when prop on serviceAPI is undefined', () => {
            const configProviderModule = require('../configProvider');
            const configProviderService = configProviderModule.default;
            const { getFormFactor } = configProviderModule;
            configProviderService({
