...
describe('lightning-map', () => {
    ...
    it('renders map with one marker and listview always visible', () => {
        const map = createMap({
            mapMarkers: SINGLE_MARKER,
            listView: 'visible',
        });
        ...
        return Promise.resolve().then(() => {
            ...
            expect(coordinatesList.children).toHaveLength(1);
        });
    });

    it('renders map with two markers and listview always hidden', () => {
        const map = createMap({
            mapMarkers: MULTIPLE_MARKERS,
            listView: 'hidden',
        });
        ...
        return Promise.resolve().then(() => {
            expect(map.classList).toContain('slds-grid');
            expect(map.classList).not.toContain('slds-has-coordinates');
            expect(map.classList).not.toContain('coordinates__list');
        });
    });
